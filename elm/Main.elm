...
studentCenterMailboxRegex = Maybe.withDefault Regex.never (Regex.fromString "^\\d{6} georgia tech station$")
...
emailFeedbackText = "Please enter a valid email address ending in @gatech.edu or @robojackets.org"
...
microsoftIcon = svg [ Svg.Attributes.width "16", Svg.Attributes.height "16", Svg.Attributes.viewBox "0 0 16 16", Svg.Attributes.fill "currentColor", Svg.Attributes.style "top: -0.125em; position: relative;"] [ path [ d "M7.462 0H0v7.19h7.462V0zM16 0H8.538v7.19H16V0zM7.462 8.211H0V16h7.462V8.211zm8.538 0H8.538V16H16V8.211z"] []]

checkIcon : Svg msg
checkIcon = svg [ Svg.Attributes.width "16", Svg.Attributes.height "16", Svg.Attributes.viewBox "0 0 16 16", Svg.Attributes.fill "currentColor", Svg.Attributes.style "top: -0.125em; position: relative;"] [ path [ d "M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425a.247.247 0 0 1 .02-.022Z"] []]
...
emailProviderName = Dict.fromList [("gatech.edu", "Microsoft"), ("robojackets.org", "Google")]
...
update msg model =
  case msg of
    ...
    FormSubmitted ->
      ( { model | showValidation = True, nextAction = (
    ...
    FirstNameInput firstName ->
      ( { model | firstName = firstName, nextAction = NoOpNextAction }, Cmd.none )

    LastNameInput lastName ->
      ( { model | lastName = lastName, nextAction = NoOpNextAction }, Cmd.none )

    EmailAddressInput emailAddress ->
      ( { model | emailAddress = emailAddress, emailVerified = False, nextAction = NoOpNextAction }, Cmd.none )

    EmailVerificationButtonClicked ->
      ( { model | nextAction = RedirectToEmailVerification }, saveToLocalStorage (stringifyModel model) )

    ManagerInput managerId ->
      ( { model | managerId = Just managerId, nextAction = NoOpNextAction }, saveToLocalStorage (stringifyModel { model | managerId = Just managerId }) )

    OrderPhysicalCardChecked orderPhysicalCard ->
      ( { model | orderPhysicalCard = orderPhysicalCard, nextAction = NoOpNextAction }, saveToLocalStorage (stringifyModel { model | orderPhysicalCard = orderPhysicalCard }) )

    UspsFirstClassChecked selected ->
      ( { model | shippingOption = UspsFirstClass, nextAction = NoOpNextAction }, saveToLocalStorage (stringifyModel { model | shippingOption = UspsFirstClass }) )

    FedEx2DayChecked selected ->
      ( { model | shippingOption = FedEx2Day, nextAction = NoOpNextAction }, saveToLocalStorage (stringifyModel { model | shippingOption = FedEx2Day }) )

    FedExOvernightChecked selected ->
      ( { model | shippingOption = FedExOvernight, nextAction = NoOpNextAction }, saveToLocalStorage (stringifyModel { model | shippingOption = FedExOvernight }) )

    AddressLineOneInput addressLineOne ->
      ( { model | addressLineOne = addressLineOne, nextAction = NoOpNextAction, addressLineTwoRequired = False }, Cmd.none )

    AddressLineTwoInput addressLineTwo ->
      ( { model | addressLineTwo = addressLineTwo, nextAction = NoOpNextAction }, Cmd.none )

    CityInput city ->
      ( { model | city = city, nextAction = NoOpNextAction, addressLineTwoRequired = False }, Cmd.none )

    StateInput state ->
      ( { model | state = Just state, nextAction = NoOpNextAction, addressLineTwoRequired = False }, saveToLocalStorage (stringifyModel { model | state = Just state }) )

    ZipInput zip ->
      ( { model | zip = zip, nextAction = NoOpNextAction, addressLineTwoRequired = False }, Cmd.none )

    AcknowledgeCardPolicyChecked acknowledgedCardPolicy ->
      ( { model | acknowledgedCardPolicy = acknowledgedCardPolicy, nextAction = NoOpNextAction }, Cmd.none )

    AcknowledgeReimbursementPolicyChecked acknowledgedReimbursementPolicy ->
      ( { model | acknowledgedReimbursementPolicy = acknowledgedReimbursementPolicy, nextAction = NoOpNextAction }, Cmd.none )

    AcknowledgeIdentityVerificationPolicyChecked acknowledgedIdentityVerificationPolicy ->
      ( { model | acknowledgedIdentityVerificationPolicy = acknowledgedIdentityVerificationPolicy, nextAction = NoOpNextAction }, Cmd.none )

    NoOpMsg ->
      ( { model | nextAction = NoOpNextAction }, Cmd.none )

    LocalStorageSaved _ ->
      ( { model | nextAction = NoOpNextAction, addressValidationInProgress = (
        ...
        case model.nextAction of
          RedirectToEmailVerification ->
            Nav.load (Url.Builder.absolute [ "verify-email" ] [ Url.Builder.string "emailAddress" model.emailAddress ])
          ValidateAddressWithGoogle ->
            post
              { url = Url.Builder.crossOrigin "https://addressvalidation.googleapis.com/v1:validateAddress" [] [ Url.Builder.string "key" model.googleMapsApiKey ]
              , body = jsonBody (Json.Encode.object
                  ...
                  , ("address", Json.Encode.object
                    ...
                    , ("addressLines", Json.Encode.list Json.Encode.string (List.map String.trim (List.filter nonBlankString
    ...
    PlaceChanged value ->
      (
        { model
        | addressLineOne = (String.trim (getAddressComponent (decodePlaceChanged value) "street_number")) ++ " " ++ (String.trim (getAddressComponent (decodePlaceChanged value) "route"))
    ...
    GoogleAddressValidationResultReceived result ->
      ( { model | nextAction = NoOpNextAction, addressValidationInProgress = False, addressLineTwoRequired = (
          case result of
            Ok verdict ->
...
view model =
  ...
  , body =
      [ div [ class "container", class "mt-md-4", class "mt-3", style "max-width" "48rem" ]
        ...
        , Html.form [ class "row", class "g-3", method "POST", action "/", novalidate True, onSubmit FormSubmitted ]
          [ div [ class "col-6" ]
          ...
          , div [ class "col-12" ]
            ...
            , div [ class "input-group" ]
              ...
              , button [
                  ...
                  disabled ((model.emailVerified || (not (Dict.member (withDefault "unknown" (emailAddressDomain model.emailAddress)) emailProviderName))) || (model.nextAction /= NoOpNextAction)),
                  ...
                  , text (noBreakSpace ++ noBreakSpace ++ (
                      if model.emailVerified then
                        "Verified"
                      else if (Dict.member (withDefault "unknown" (emailAddressDomain model.emailAddress)) emailProviderName) then
                        "Verify with " ++ withDefault "Unknown" (Dict.get (withDefault "unknown" (emailAddressDomain model.emailAddress)) emailProviderName)
                      else
                        "Verify"
                    ))
                ]
          ...
          , div [ class "col-12" ]
            ...
            , select [
              ...
              ] ([ option [ Html.Attributes.value "", disabled True, selected (
          ...
          , div [ class "col-12", classList [("d-none", not model.orderPhysicalCard)] ]
            ...
            , input [ type_ "text", class "form-control",
                  ...
                  ], id "address_line_one", name "address_line_one", minlength 1, maxlength 100, required True, placeholder "Street Address", onInput AddressLineOneInput,
                  on "change" (succeed FormChanged), Html.Attributes.value model.addressLineOne, preventDefaultOn "keypress" keyDecoder ] []
            , div [ class "invalid-feedback" ]
              [ text (
          ...
          , div [ class "col-12", classList [("d-none", not model.orderPhysicalCard)] ]
            [ input [ type_ "text", class "form-control",
                  ...
                  ], id "address_line_two", name "address_line_two", maxlength 100, placeholder "Apt, Suite, Unit, etc. (optional)", onInput AddressLineTwoInput,
                  on "change" (succeed FormChanged), Html.Attributes.value model.addressLineTwo ] []
            , div [ class "invalid-feedback" ]
          ...
          , div [ class "col-md-6", classList [("d-none", not model.orderPhysicalCard)] ]
            ...
            , input [ type_ "text", class "form-control",
                  ...
                  ], id "city", name "city", minlength 1, maxlength 40, placeholder "City", required True, onInput CityInput,
                  on "change" (succeed FormChanged), Html.Attributes.value model.city ] []
            , div [ class "invalid-feedback" ]
          ...
          , div [ class "col-md-3", class "col-8", classList [("d-none", not model.orderPhysicalCard)] ]
            ...
            , select [ class "form-select", id "state", name "state", minlength 1, maxlength 40, required True,
                classList [
          ...
          , div [ class "col-md-3", class "col-4", class "mb-3", classList [("d-none", not model.orderPhysicalCard)] ]
            ...
            , input [ type_ "text", toAttribute(inputmode numeric), class "form-control", id "zip_code", name "zip_code", placeholder "ZIP Code", minlength 5, maxlength 5, required True, onInput ZipInput,
                  on "change" (succeed FormChanged), Html.Attributes.value model.zip,
                classList [
          ...
          , div [ class "col-12", class "mb-3" ]
            [ div [ class "form-check", class "mb-2" ]
              [ input [ class "form-check-input", type_ "checkbox", id "corporate_card_policy", required True, onCheck AcknowledgeCardPolicyChecked,
                classList [
            ...
            , div [ class "form-check", class "mb-2" ]
              [ input [ class "form-check-input", type_ "checkbox", id "reimbursement_policy", required True, onCheck AcknowledgeReimbursementPolicyChecked,
                classList [
            ...
            , div [ class "form-check", class "mb-2" ]
              [ input [ class "form-check-input", type_ "checkbox", id "identity_verification_policy", required True, onCheck AcknowledgeIdentityVerificationPolicyChecked,
                classList [
          ...
          , div [ class "col-12", class "mb-3", class "mb-md-5" ]
            [ button [ type_ "submit", class "btn", class "btn-primary", id "submit_button", disabled (model.nextAction /= NoOpNextAction || model.addressValidationInProgress || model.formSubmissionInProgress) ]
              [ text "Submit Request"
              ]
            ]
          ]
...
validateAddressLineTwo addressLineTwo required campusAddress =
  ...
  else if campusAddress == StudentCenter && not (Regex.contains studentCenterMailboxRegex (String.trim (String.toLower (addressLineTwo)))) then
    Err "This doesn't appear to be a valid mailbox number"
  else if campusAddress == GraduateLivingCenter && not (Regex.contains graduateLivingCenterMailboxRegex (String.trim (String.toLower (addressLineTwo)))) then
...
validateModel model =
  ...
  else if (model.orderPhysicalCard && not (isValid (validateAddressLineTwo model.addressLineTwo model.addressLineTwoRequired (checkCampusAddress model)))) then
    Err "address_line_two"
  else if (model.orderPhysicalCard && not (isValid (validateCity(model.city)))) then
    Err "city"
  else if (model.orderPhysicalCard && (
    case model.state of
      Just _->
        False
      Nothing ->
...
decodePlaceChanged value =
  Result.withDefault [] (decodeValue (field "address_components" (Json.Decode.list (
...
checkCampusAddress model =
  if (String.toLower (String.trim model.addressLineOne)) == "351 ferst dr nw" &&
    (String.toLower (String.trim model.city)) == "atlanta" &&
    Maybe.withDefault "" model.state == "GA" &&
    (String.trim model.zip) == "30332"
  then
    StudentCenter
  else if (String.toLower (String.trim model.addressLineOne)) == "301 10th st nw" &&
    (String.toLower (String.trim model.city)) == "atlanta" &&
    Maybe.withDefault "" model.state == "GA" &&
    (String.trim model.zip) == "30318"
  then
    GraduateLivingCenter
  else
    NotCampusAddress
